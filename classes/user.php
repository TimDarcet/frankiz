...
class User extends Meta
{
    ...
    */
    ...
    {
        // If we are unauthentificated, outside the platal, and trying to sneak at a internal ressource
        if(S::i('auth') < AUTH_INTERNAL) {
            $caste->group()->select(GroupSelect::base());
            if(! $caste->group()->external())
                return false;
        }
    ...
    *******************************************************************************/
    ...
    public static function getSilentWithValues($login, $values)
    {
        ...
        if ($login == 0) {
            ...
            $uid = (IPAddress::getInstance()->is_x_internal()
                ? $globals->anonymous->internal
                : $globals->anonymous->external);
            S::set('newuid', $uid);
            try {
                $u = new User($uid);
