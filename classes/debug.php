...
abstract class Debug
{
    ...
    /**
     * @desc call with only one parameter
     * @param mixed [optional]
     * @return null
     */
    public static function w($arg)
    {
        $glue    = PHP_EOL . PHP_EOL;
        $var     = self::_getVariable($arg);
        $trace[] = self::_getBacktrace($glue, 1);
        $trace[] = self::_getMemoryUsage();
        $trace[] = self::_getTimestamp();
        $trace   = implode($glue, $trace);

        self::$postflush[] = array('var' => $var, 'trace' => $trace);
    }
