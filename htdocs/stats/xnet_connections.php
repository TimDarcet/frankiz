...
foreach ($os as $nom => $nombre) {
		...
		ImageFilledRectangle ($im, $i*$largeur/(count($os)+1), $hauteur-$hauteurImageRectangle, ($i+1)*$largeur/(count($os)+1), $hauteur-41, $noir);
		ImageFilledRectangle ($im, $i*$largeur/(count($os)+1)+2, $hauteur-$hauteurImageRectangle+2, ($i+1)*$largeur/(count($os)+1)-2, $hauteur-41-1, $bleu_fonce);
