...
foreach ($_POST AS $keys => $val){
	...
	if ($keys=='rajoutlien') {
		$DB_web->query("INSERT INTO liens SET description='".$_REQUEST['explilien']."', titre='".$_REQUEST['titrelien']."', url='".$_REQUEST['urllien']."'");
	}
...
<?
		$DB_web->query("SELECT lien_id,titre,url,description FROM liens ORDER BY titre") ;
		while(list($lien_id,$titre,$url,$description) = $DB_web->next_row()) {
