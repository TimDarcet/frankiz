...
function conv_date($datestr)
{
    return substr($datestr, 6)."-".substr($datestr, 3, 2)."-".substr($datestr, 0, 2);
}
...
//    $u->nickname(conv($datas['surnom']));
    //echo(conv_date($datas[$birthdate]));
    $u->birthdate(new FrankizDateTime(conv_date($datas[$birthdate])));
    if($gender != null)
