 ...
 */
set_time_limit(0);
$file = "/home/2011/levans/EV2_import.csv";
$photos_folder = "";
...
//    $u->nickname(conv($datas['surnom']));
    ...
    if($gender != null)
        $u->gender(($datas[$gender] == 'Melle') ? User::GENDER_FEMALE : User::GENDER_MALE);
...
*/
    ...
    */
    ...
    $u->addStudy($formation_id, ($year_in === null ? 2012 : (int) $datas[$year_in]), ($year_out === null ? 2012 + 4 : (int) $datas[$year_out]), 2012, $login);
   ...
   /* if ($promo != null) {
        $nf = new GroupFilter(new GFC_Name('promo_' . conv_name(2012)));
        $n = $nf->get(true);
        if($n){
            $n->select(GroupSelect::castes());
            $n->caste(Rights::member())->addUser($u);
