 ...
 */
set_time_limit(0);
$file = "/home/2013/ntag/BR_X2014_Logement_email.csv";
//$photos_folder = "";
...
function conv_mail($str)
{
    $str = utf8_encode(strtr(utf8_decode($str), utf8_decode('ÁÀÂÄÃÅÇÉÈÊËÍÏÎÌÑÓÒÔÖÕÚÙÛÜÝ'), 'AAAAAACEEEEEIIIINOOOOOUUUUY'));
    $str = strtolower(conv($str));
    $str = utf8_encode(strtr(utf8_decode($str), utf8_decode('áàâäãåçéèêëíìîïñóòôöõúùûüýÿ'), 'aaaaaaceeeeiiiinooooouuuuyy'));
    $str = strtolower($str);
...
/* Only for new users : none expected
...
//print_r($datas);
    ...
    $res = XDB::query('SELECT uid FROM account WHERE email = {?}', $emailFinal);
    if ($res->numRows() == 0)
    {   
    	echo 'Failure : user'. $datas[$firstname] .' '. $datas[$lastname].' doesn\'t exist';
	continue;
	$u -> insert();
	$new = 1;
    }
...
//    $u->addStudy($formation_id, ($year_in === null ? 2014 : (int) $datas[$year_in]), ($year_out === null ? 2014 + 4 : (int) $datas[$year_out]), 2014, $login);
...
*/
    ...
    echo 'User ' . str_pad($k, 4, '0', STR_PAD_LEFT) . '/xx : '
         ...
         . substr(microtime(true) - $t, 0, 5) . '   ' . $emailFinal . "\n";
}
