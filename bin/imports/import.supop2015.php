...
$f = fopen(dirname(__FILE__) . '/supop2015.csv', 'rb') or die("No data file\n");
...
while (!feof($f)) {
    ...
    if ($data === false)
        continue;
    list($promo, $hruid, $lastname, $firstname, $email, $sex) = $data;
    ...
    $email = $hruid . "@institutoptique.fr";
    if (isset($accounts_studies[$hruid])) {
        ...
        print("Error: user $hruid already exists !\n");
    } else {
        // Create user
        $u = create_user(array(
            'hruid' => $hruid,
